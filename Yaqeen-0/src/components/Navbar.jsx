...
const Navbar = () => {
  ...
  return (
    ...
    <motion.nav 
      ...
      className={`fixed top-0 z-40 w-full backdrop-blur-xl transition-colors duration-500 border-b ${theme === 'dark' ? 'bg-[#0f1c2c]/80 border-[#d6a54a]/20' : 'bg-[#FAF8F5]/80 border-[#0f1c2c]/10 shadow-sm'}`}
    >
        ...
        <div className="flex shrink-0 items-center gap-4">
          ...
          <button 
            ...
            className={`p-3 rounded-full border transition-all hover:scale-105 flex items-center justify-center
              ${theme === 'dark' ? 'bg-[#d6a54a]/10 border-[#d6a54a]/20 text-[#d6a54a] hover:bg-[#d6a54a] hover:text-[#0A1A14]' : 'bg-[#0f1c2c]/5 border-[#0f1c2c]/20 text-[#0f1c2c] hover:bg-[#0f1c2c] hover:text-[#FAF8F5]'}
            `}
            title="بحث"
          >
            <Search size={22} />
          ...
          {links.map((link) => {
            ...
            return (
                ...
                className={`relative px-4 py-2 transition-all duration-300 font-bold z-10 whitespace-nowrap flex items-center gap-1.5 
                  ${isActive 
                    ? (theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c] font-black')
                    : (theme === 'dark' ? 'text-[#FAF8F5]/70 hover:text-[#d6a54a]' : 'text-[#0f1c2c]/60 hover:text-[#0f1c2c]')
                  }
                `}
              >
                {isActive && (
                  <motion.div 
                    ...
                    className={`absolute -bottom-2 inset-x-0 mx-auto h-1 rounded-full ${theme === 'dark' ? 'bg-[#d6a54a] shadow-[0_0_10px_rgba(198,156,109,0.6)] w-full' : 'bg-[#0f1c2c] w-full'}`}
    ...
    <nav className={`md:hidden fixed bottom-14 left-0 right-0 z-40 backdrop-blur-xl border-t shadow-[0_-10px_40px_rgba(0,0,0,0.1)] pb-[env(safe-area-inset-bottom)]
       ${theme === 'dark' ? 'bg-[#0A1A14]/90 border-[#d6a54a]/20 text-[#FAF8F5]' : 'bg-white/90 border-[#0f1c2c]/10 text-[#0f1c2c]'}
    `}>
         ...
         {mobileTopLinks.map((link) => {
            ...
            return (
              <Link key={link.path} to={link.path} className={`flex flex-col items-center justify-center w-full h-full gap-1 transition-all
                  ${isActive ? (theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]') : 'opacity-50'}
              `}>
      ...
      {isMoreOpen && (
        <motion.div 
          ...
          className={`md:hidden fixed inset-x-0 bottom-0 z-50 h-[85vh] rounded-t-3xl shadow-2xl flex flex-col pt-6 font-tajawal
             ${theme === 'dark' ? 'bg-[#0A1A14] text-white border-t border-[#d6a54a]/30' : 'bg-[#FAF8F5] text-[#0f1c2c] border-t border-[#0f1c2c]/20'}
          `}
        >
    ...
    <div className="fixed right-4 top-1/2 -translate-y-1/2 z-[60] hidden lg:flex flex-col items-center gap-6">
      <div className={`w-px h-20 bg-gradient-to-b from-transparent ${theme === 'dark' ? 'via-[#d6a54a]/40' : 'via-[#0f1c2c]/40'} to-transparent mb-4`}></div>
      {links.map((link, idx) => {
        const isActive = isActivePath(link.path);
        return (
          <Link key={idx} to={link.path} className="group relative">
             <motion.div 
               ...
               className={`w-2 h-2 rounded-full border transition-all ${isActive 
                 ? (theme === 'dark' ? 'bg-[#d6a54a] border-[#d6a54a]' : 'bg-[#0f1c2c] border-[#0f1c2c]')
                 : (theme === 'dark' ? 'bg-transparent border-[#d6a54a]/30 group-hover:border-[#d6a54a]' : 'bg-transparent border-[#0f1c2c]/30 group-hover:border-[#0f1c2c]')
               }`}
             />
             <span className={`absolute right-6 top-1/2 -translate-y-1/2 px-3 py-1 rounded-md text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all pointer-events-none font-bold
               ${theme === 'dark' ? 'bg-[#d6a54a] text-[#0A1A14]' : 'bg-[#0f1c2c] text-white'}
             `}>
               {link.label}
             </span>
          </Link>
      ...
      <div className={`w-px h-20 bg-gradient-to-t from-transparent ${theme === 'dark' ? 'via-[#d6a54a]/40' : 'via-[#0f1c2c]/40'} to-transparent mt-4`}></div>
    </div>
    </>
  );
};
