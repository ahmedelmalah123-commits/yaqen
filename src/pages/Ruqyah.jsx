  ...
  <div className={`p-8 rounded-[2rem] border-2 shadow-xl ${theme === 'dark' ? 'bg-[#064e3b] border-primary/20 text-white' : 'bg-white border-secondary/10 text-secondary'}`}>
    ...
    <div className="space-y-6">
      {adhkars.map((dhikr, idx) => (
         <div key={idx} className={`p-6 rounded-2xl border transition-all ${theme === 'dark' ? 'bg-black/20 border-primary/10 hover:border-primary/40' : 'bg-gray-50 border-secondary/5 hover:border-secondary/30'}`}>
  ...
  return (
    <div className="py-12 max-w-6xl mx-auto px-4 font-readex pb-40">
      ...
      `}>
        ...
        <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-transparent via-primary/5 to-transparent"></div>
        <div className="relative z-10">
          <h1 className={`text-4xl md:text-7xl font-bold mb-6 drop-shadow-lg font-cairo tracking-wide ${theme === 'dark' ? 'text-primary' : 'text-secondary'}`}>
            حصن المسلم
          </h1>
        ...
        <button 
          ...
          className={`px-8 py-4 rounded-full font-bold text-xl transition-all shadow-md flex items-center gap-3 border
            ${activeTab === 'ruqyah' 
               ...
               : (theme === 'dark' ? 'bg-transparent text-primary border-primary/40 hover:bg-primary/10' : 'bg-transparent text-secondary border-secondary/40 hover:bg-secondary/10')
            }
          `}
