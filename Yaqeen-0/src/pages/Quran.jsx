  ...
  return (
    <div className="py-6 md:py-10 max-w-7xl mx-auto px-4 font-tajawal pb-32">
      ...
      <div className="flex flex-col gap-5 mb-8 md:mb-10">
        <div>
          <h1 className={`text-3xl md:text-5xl font-bold mb-2 ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>
            القرآن الكريم
          </h1>
          <p className={`text-base md:text-lg opacity-70 ${theme === 'dark' ? 'text-[#FAF8F5]' : 'text-[#0f1c2c]'}`}>
          ...
          <div className="relative flex-1">
            <input
              ...
              className={`w-full h-12 py-3 pr-11 pl-4 rounded-2xl outline-none transition-all border text-sm md:text-base glass-card
                ${theme === 'dark'
                  ? 'border-[#d6a54a]/30 text-[#FAF8F5] focus:border-[#d6a54a] placeholder-[#FAF8F5]/40'
                  : 'border-[#0f1c2c]/20 text-[#0f1c2c] focus:border-[#0f1c2c] placeholder-[#0f1c2c]/40'
                }
              `}
              dir="rtl"
            />
            <Search className={`absolute right-4 top-1/2 -translate-y-1/2 ${theme === 'dark' ? 'text-[#d6a54a]/60' : 'text-[#0f1c2c]/60'}`} size={18} />
          </div>
          ...
          <div className="relative">
            <button
              ...
              className={`h-12 flex items-center gap-2 px-4 rounded-2xl border font-bold text-sm whitespace-nowrap transition-all w-full sm:w-auto glass-card
                ${theme === 'dark'
                  ? 'border-[#d6a54a]/40 text-[#d6a54a] hover:bg-[#d6a54a]/10'
                  : 'border-[#0f1c2c]/30 text-[#0f1c2c] hover:bg-[#0f1c2c]/5'
                }
              `}
            >
              <Mic2 size={16} />
                    ...
                    return (
                      <div key={group}>
                        <div className={`px-4 py-1.5 text-xs font-bold tracking-wide sticky top-0
                          ${theme === 'dark' ? 'bg-[#0f1c2c]/80 text-[#d6a54a]/70' : 'bg-[#FAF8F5]/90 text-[#0f1c2c]/60'}
                        `}>
                          {group}
                        </div>
                        {groupReciters.map(r => (
                          <button
                            ...
                            className={`w-full text-right px-4 py-2.5 text-sm font-medium transition-colors flex items-center justify-between gap-2
                              ${r.id === reciter
                                ? (theme === 'dark' ? 'bg-[#d6a54a]/20 text-[#d6a54a]' : 'bg-[#0f1c2c]/10 text-[#0f1c2c] font-bold')
                                : (theme === 'dark' ? 'text-[#FAF8F5]/80 hover:bg-white/5' : 'text-[#0f1c2c]/70 hover:bg-black/5')
                              }
                            `}
                          >
      ...
      {loading ? (
        <div className="auto-grid">
          {[...Array(16)].map((_, i) => (
            <div key={i} className={`h-20 rounded-2xl animate-pulse ${theme === 'dark' ? 'bg-[#0c1622]' : 'bg-white shadow-sm'}`} />
          ))}
        </div>
      ) : (
        <motion.div
            ...
            <Link key={surah.number} to={`/surah/${surah.number}`} onClick={() => setReciterOpen(false)}>
              ...
              >
                <div className={`absolute -right-4 -top-4 w-16 h-16 rounded-full blur-2xl opacity-20 transition-all group-hover:opacity-60 group-hover:scale-150
                  ${theme === 'dark' ? 'bg-[#d6a54a]' : 'bg-[#0f1c2c]'}
                `}></div>
                ...
                <div className={`w-10 h-10 md:w-11 md:h-11 flex-shrink-0 rounded-xl flex items-center justify-center font-bold text-sm border rotate-45 z-10
                  ${theme === 'dark' ? 'border-[#d6a54a]/40 text-[#d6a54a]' : 'border-[#0f1c2c]/30 text-[#0f1c2c]'}
                `}>
                ...
                <div className="flex-1 min-w-0 z-10">
                  <h3 className={`text-lg md:text-xl font-bold truncate leading-tight font-amiri ${theme === 'dark' ? 'text-white' : 'text-[#0f1c2c]'}`}>
                    {surah.name.replace(/س(?:ُورَةُ|ورة)\s*/g, 'سورة ')}
                  </h3>
                  <p className={`text-xs md:text-sm mt-0.5 ${theme === 'dark' ? 'text-[#FAF8F5]/60' : 'text-[#0f1c2c]/60'}`}>
      ...
      {!loading && filteredSurahs.length === 0 && (
        <div className={`text-center py-20 opacity-60 font-bold text-xl ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>
