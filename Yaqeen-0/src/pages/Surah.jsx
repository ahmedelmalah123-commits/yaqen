...
const CinematicOverlay = ({ surahData, audioState, user, onClose, onPlayPause, onNext, onPrev }) => {
  ...
  return (
    <div className="fixed inset-0 z-[500] flex flex-col items-center justify-center overflow-hidden bg-[#0A1A14]">
      {/* Animated background */}
      <motion.div
        animate={{
          background: [
            "radial-gradient(circle at 10% 20%, rgba(200,169,106,0.25) 0%, rgba(10,26,20,1) 70%)",
            "radial-gradient(circle at 90% 80%, rgba(200,169,106,0.15) 0%, rgba(10,26,20,1) 80%)",
            "radial-gradient(circle at 50% 10%, rgba(200,169,106,0.2) 0%, rgba(5,15,10,1) 60%)",
            "radial-gradient(circle at 10% 20%, rgba(200,169,106,0.25) 0%, rgba(10,26,20,1) 70%)",
          ]
        }}
      ...
      <div className="absolute top-6 left-6 right-6 flex justify-between items-center z-50">
        ...
        <div className="px-5 py-2.5 rounded-full bg-[#d6a54a]/10 text-[#d6a54a] backdrop-blur-md border border-[#d6a54a]/20 font-bold tracking-widest text-base flex items-center gap-3 font-reem">
          <div className="w-2 h-2 rounded-full bg-[#d6a54a] animate-pulse" />
          وضع الاستماع
        </div>
        <div className="w-14" />
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <h3 className="text-[#d6a54a] font-kufi text-xl md:text-2xl mb-8 tracking-widest opacity-90 border-b border-[#d6a54a]/30 pb-3 font-reem">
            سورة {surahData.name.replace('سُورَةُ ', '')}
          </h3>
          <h2 className="text-[#FAF8F5] font-amiri text-3xl md:text-6xl leading-[2] md:leading-[2.2] tracking-wide drop-shadow-[0_0_25px_rgba(198,156,109,0.35)]" style={{ wordSpacing: '6px' }}>
            {ayahText}
            <span className="inline-flex items-center justify-center mx-4 text-2xl md:text-4xl font-black text-[#d6a54a] opacity-60">
      ...
      <div className="absolute bottom-16 flex items-center gap-6 md:gap-8 z-50">
        <button onClick={handleBookmark} disabled={isSaving || isSaved} className={`p-4 rounded-full bg-white/5 hover:bg-white/10 backdrop-blur-md transition-all ${isSaved ? 'text-green-400' : 'text-[#d6a54a]'} disabled:opacity-50`}>
          <Bookmark size={24} className={isSaved ? "fill-current" : ""} />
        </button>

        <button onClick={onPrev} disabled={currentIndex === 0} className="p-3 rounded-full bg-white/5 hover:bg-white/15 text-[#d6a54a] backdrop-blur-md transition-all disabled:opacity-30">
          <SkipForward size={24} />
        </button>
        <motion.button
          whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
          onClick={onPlayPause}
          className="w-20 h-20 rounded-full border-2 border-[#d6a54a]/40 flex items-center justify-center text-[#d6a54a] hover:bg-[#d6a54a] hover:text-[#0A1A14] transition-all shadow-[0_0_30px_rgba(198,156,109,0.3)] backdrop-blur-md"
        >
          {isPlaying
        ...
        <button onClick={onNext} disabled={currentIndex >= surahData.ayahs.length - 1} className="p-3 rounded-full bg-white/5 hover:bg-white/15 text-[#d6a54a] backdrop-blur-md transition-all disabled:opacity-30">
          <SkipBack size={24} />
        </button>
      ...
      <div className="absolute bottom-8 left-12 right-12 flex items-center gap-3 z-50">
        <span className="text-[#d6a54a]/60 text-sm font-tajawal">{(currentIndex + 1).toLocaleString('ar-EG')}</span>
        <div className="flex-1 h-1 bg-white/10 rounded-full overflow-hidden">
          <motion.div
            className="h-full bg-gradient-to-r from-[#d6a54a]/60 to-[#d6a54a] rounded-full"
            animate={{ width: `${((currentIndex + 1) / surahData.ayahs.length) * 100}%` }}
            transition={{ duration: 0.5 }}
          />
        </div>
        <span className="text-[#d6a54a]/60 text-sm font-tajawal">{surahData.ayahs.length.toLocaleString('ar-EG')}</span>
      </div>
    </div>
  );
};
...
const Surah = () => {
  ...
  useEffect(() => {
    if (surahData && typeof setSurahInfo === 'function') {
  ...
  if (loading && !surahData) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-6">
       <div className={`animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 ${theme === 'dark' ? 'border-[#d6a54a]' : 'border-[#0f1c2c]'}`}></div>
       <p className={`font-tajawal text-xl opacity-80 font-medium ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>
         جاري إعداد السورة...
       </p>
    </div>
  );
  ...
  if (error && !surahData) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-6 text-center px-4 font-tajawal">
      ...
      <button 
        ...
        className={`px-8 py-3 rounded-full font-bold transition-all shadow-lg ${theme === 'dark' ? 'bg-[#d6a54a] text-[#0c1622]' : 'bg-[#0f1c2c] text-white'}`}
      >
        إعادة المحاولة
      </button>
    </div>
  ...
  return (
    <div className="py-6 md:py-12 max-w-6xl mx-auto px-4 font-tajawal pb-32 md:pb-40">
      ...
      <div className={`relative rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 mb-8 md:mb-10 overflow-hidden text-center glass-card transition-colors duration-500
        ${theme === 'dark' ? 'text-white' : 'text-[#0f1c2c]'}
      `}>
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="w-full flex justify-between items-center mb-8">
             <Link to="/quran" className={`p-4 rounded-full transition-all border ${theme === 'dark' ? 'hover:bg-[#d6a54a]/10 border-transparent hover:border-[#d6a54a]/30' : 'hover:bg-white border-transparent hover:border-[#0f1c2c]/20'}`}>
               <ArrowRight size={28} className={theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'} />
             </Link>
             <div className="flex-1 text-center">
               <span className={`text-xl font-medium tracking-widest ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>
          ...
          <h1 className={`text-5xl md:text-[6rem] font-bold mb-4 md:mb-6 drop-shadow-lg font-reem ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>
            سورة {surahData.name.replace('سُورَةُ ', '')}
          </h1>
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-8 md:mb-10 w-full justify-center px-4 md:px-0">
             <button 
               ...
               className={`flex items-center justify-center gap-2 md:gap-3 px-6 md:px-8 py-3 md:py-4 rounded-full font-bold text-lg md:text-xl transition-all shadow-xl hover:scale-105 w-full md:w-auto
                 ${theme === 'dark' ? 'bg-[#d6a54a] text-[#0A1A14] hover:bg-white' : 'bg-[#0f1c2c] text-[#FAF8F5] hover:bg-black'}
               `}
             >
             ...
             <button 
               ...
               className={`flex items-center justify-center gap-2 md:gap-3 px-6 md:px-8 py-3 md:py-4 rounded-full font-bold text-lg md:text-xl transition-all border shadow-lg hover:scale-105 w-full md:w-auto
                 ...
                 ${showTafsir && (theme === 'dark' ? 'bg-[#d6a54a]/20' : 'bg-[#0f1c2c]/10')}
               `}
             >
          ...
          <div className="w-full max-w-4xl mx-auto flex flex-col items-center border-t border-dashed pt-8 mt-4 border-[#d6a54a]/30">
             <div className="w-full space-y-6">
                {reciterGroups.map(group => (
                  <div key={group.group} className="flex flex-col md:flex-row items-center justify-center gap-4">
                     <span className={`text-sm font-bold opacity-70 w-32 text-center md:text-right ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>
                        {group.group}:
                     </span>
                          ...
                          return (
                            <button 
                              ...
                              className={`px-4 py-2 rounded-xl text-md font-bold transition-all border
                                ${isSelected 
                                  ? (theme === 'dark' ? 'bg-[#d6a54a] text-[#0A1A14] shadow-[0_0_15px_rgba(198,156,109,0.5)] border-[#d6a54a]' : 'bg-[#0f1c2c] text-white shadow-lg border-[#0f1c2c]') 
                                  : (theme === 'dark' ? 'bg-transparent text-[#FAF8F5] border-[#d6a54a]/20 hover:border-[#d6a54a]' : 'bg-transparent text-[#0f1c2c] border-[#0f1c2c]/20 hover:border-[#0f1c2c]')
                                }
                              `}
                            >
                              {r.name}
      ...
      {surahData.number !== 1 && surahData.number !== 9 && (
        <div className={`text-center text-3xl md:text-4xl mb-8 md:mb-12 font-bold leading-relaxed tracking-wide font-reem ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>
      ...
      {!showTafsir ? (
        ...
        <div className={`p-5 sm:p-8 md:p-14 lg:p-20 rounded-[2rem] md:rounded-[3rem] min-h-[100px] text-justify leading-[2.6] md:leading-[3.5] tracking-wide glass-card transition-colors duration-500
            ${theme === 'dark' ? 'text-[#FAF8F5]' : 'text-[#0f1c2c]'}
        `} dir="rtl">
              ...
              return (
                <span 
                  ...
                  className={`inline cursor-pointer transition-colors duration-300 mx-1 px-1 py-1 rounded-lg
                    ${isCurrent 
                      ? (theme === 'dark' ? 'text-[#d6a54a] font-black drop-shadow-md bg-[#d6a54a]/10' : 'text-[#0f1c2c] font-black drop-shadow-md bg-[#0f1c2c]/10') 
                      : (theme === 'dark' ? 'hover:text-[#d6a54a]/80 hover:bg-[#d6a54a]/5' : 'hover:text-[#0f1c2c]/70 hover:bg-[#0f1c2c]/5')
                    }
                  `}
                >
                  {ayahText}
                  <span className={`inline-block mx-1.5 md:mx-2 font-black select-none text-xl md:text-3xl whitespace-nowrap align-middle
                    ${isCurrent ? (theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]') : (theme === 'dark' ? 'text-[#d6a54a]/50' : 'text-[#0f1c2c]/50')}
                  `}>
                    ﴿ {ayah.numberInSurah.toLocaleString('ar-EG')} ﴾
                  </span>
                </span>
            ...
            return (
              <motion.div 
                ...
                className={`p-6 md:p-10 rounded-3xl transition-all flex flex-col gap-6 md:gap-8 glass-card group
                  ${isCurrent 
                    ? (theme === 'dark' 
                        ? 'border-[#d6a54a]/50 shadow-[0_0_30px_rgba(198,156,109,0.15)] scale-[1.01] bg-[#0f1c2c]/20' 
                        : 'border-[#0f1c2c]/50 shadow-xl scale-[1.01] bg-white')
                    : ''
                  }
                `}
              >
                ...
                <div className="w-full text-center md:text-right">
                   <p className={`text-3xl md:text-[3rem] font-amiri leading-[2.2] md:leading-[2.5]
                     ${isCurrent ? (theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]') : (theme === 'dark' ? 'text-[#FAF8F5]' : 'text-[#0f1c2c]')}
                   `}>
                     {ayahText}
                     <span className={`mx-2 md:mx-4 text-xl md:text-2xl font-black ${theme === 'dark' ? 'text-[#d6a54a]/60' : 'text-[#0f1c2c]/60'}`}>
                        ﴿ {ayah.numberInSurah.toLocaleString('ar-EG')} ﴾
                     </span>
                   </p>
                </div>
                ...
                <div className={`mt-2 md:mt-4 pt-4 md:pt-6 border-t border-dashed flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-all
                    ${theme === 'dark' ? 'border-[#d6a54a]/20' : 'border-[#0f1c2c]/20'}
                `}>
                  {/* Actions */}
                  <div className="flex items-center gap-3 w-full md:w-auto justify-center md:justify-start">
                     <button 
                       ...
                       className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold text-sm md:text-base transition-all
                         ${isCurrent && isPlaying ? (theme === 'dark' ? 'bg-[#d6a54a] text-[#0A1A14]' : 'bg-[#0f1c2c] text-white') : (theme === 'dark' ? 'bg-[#d6a54a]/10 text-[#d6a54a] hover:bg-[#d6a54a] hover:text-[#0A1A14]' : 'bg-[#0f1c2c]/10 text-[#0f1c2c] hover:bg-[#0f1c2c] hover:text-white')}
                       `}
                     >
                     ...
                     <button 
                       ...
                       className={`p-2.5 rounded-full transition-all
                         ${theme === 'dark' ? 'bg-[#d6a54a]/10 text-[#d6a54a] hover:bg-[#d6a54a] hover:text-[#0A1A14]' : 'bg-[#0f1c2c]/10 text-[#0f1c2c] hover:bg-[#0f1c2c] hover:text-white'}
                       `}
                       title="احفظ الآية"
                     >
                       <Bookmark size={18} />
                  ...
                  <div className="flex items-start gap-4 flex-1">
                     <p className={`text-sm md:text-base leading-relaxed text-right flex-1 opacity-90
                        ${theme === 'dark' ? 'text-[#FAF8F5]' : 'text-gray-700'}
                     `}>
                       {tafsirText}
                     </p>
                     ...
                     <button 
                       ...
                       className={`p-2.5 rounded-full border transition-all flex-shrink-0
                         ${theme === 'dark' ? 'border-[#d6a54a]/30 text-[#d6a54a] hover:bg-[#d6a54a] hover:text-[#0A1A14]' : 'border-[#0f1c2c]/20 text-[#0f1c2c] hover:bg-[#0f1c2c] hover:text-white'}
                       `}
                       title="استمع للتفسير"
                     >
                       <Mic2 size={18} />
