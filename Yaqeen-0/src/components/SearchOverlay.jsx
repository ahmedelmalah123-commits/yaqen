        ...
        {isOpen && (
          <motion.div 
            ...
            className={`w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl border
              ${theme === 'dark' ? 'bg-[#0A1A14] border-[#d6a54a]/30' : 'bg-white border-[#0f1c2c]/20'}
            `}
          >
            {/* Search Input Header */}
            <div className={`p-4 flex items-center border-b ${theme === 'dark' ? 'border-[#d6a54a]/20' : 'border-[#0f1c2c]/10'}`}>
               <Search className={`ml-4 ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-gray-400'}`} />
               <input 
                 autoFocus
                 type="text" 
                 ...
                 className={`flex-1 bg-transparent text-xl font-bold outline-none font-tajawal
                   ${theme === 'dark' ? 'text-white placeholder:text-white/30' : 'text-[#0f1c2c] placeholder:text-gray-400'}
                 `}
               />
               <button onClick={onClose} className={`p-2 rounded-full transition-colors ${theme === 'dark' ? 'hover:bg-white/10 text-white' : 'hover:bg-black/5 text-[#0f1c2c]'}`}>
                 <X />
               </button>
            </div>
               ...
               {!query.trim() ? (
                 ...
                 <div className="space-y-6 p-4 font-tajawal">
                   ...
                   {results.surahs.length > 0 && (
                     <div>
                       <h3 className={`text-sm font-bold mb-3 opacity-70 ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>الـسـور</h3>
                         ...
                         {results.surahs.map(surah => (
                           <button 
                             ...
                             className={`w-full flex items-center justify-between p-4 rounded-xl transition-all
                               ${theme === 'dark' ? 'hover:bg-[#d6a54a]/10 text-white' : 'hover:bg-black/5 text-[#0f1c2c]'}
                             `}
                           >
                              <div className="flex items-center gap-4">
                                <span className={`p-2 rounded-lg ${theme === 'dark' ? 'bg-[#0c1622] text-[#d6a54a]' : 'bg-[#F5F5DC] text-[#0f1c2c]'}`}><Book size={20} /></span>
                   ...
                   {results.reciters.length > 0 && (
                     <div>
                       <h3 className={`text-sm font-bold mb-3 opacity-70 mt-4 ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>الـقـراء</h3>
                       <div className="space-y-2">
                         {results.reciters.map(reciter => (
                           ...
                           >
                              <div className="flex items-center gap-4">
                                <span className={`p-2 rounded-lg ${theme === 'dark' ? 'bg-[#0c1622] text-[#d6a54a]' : 'bg-[#F5F5DC] text-[#0f1c2c]'}`}><Mic size={20} /></span>
                   ...
                   {results.radios?.length > 0 && (
                     <div>
                       <h3 className={`text-sm font-bold mb-3 opacity-70 mt-4 ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]'}`}>الإذاعـات</h3>
                       <div className="space-y-2">
                         {results.radios.map(radio => (
                           ...
                           >
                              <div className="flex items-center gap-4">
                                <span className={`p-2 rounded-lg ${theme === 'dark' ? 'bg-[#0c1622] text-[#d6a54a]' : 'bg-[#F5F5DC] text-[#0f1c2c]'}`}><Radio size={20} /></span>
