    ...
    <div className="flex flex-col items-center min-h-[80vh] font-tajawal pb-32">
      ...
      >
        <div className={`relative w-full max-w-6xl mx-auto py-12 md:py-24 px-6 md:px-12 rounded-[2rem] md:rounded-[3rem] overflow-hidden glass-card
          ${theme === 'dark' ? 'bg-gradient-to-br from-[#0f1c2c]/90 to-[#0A1A14]/90 border-[#d6a54a]/30' : 'bg-gradient-to-br from-[#FAF8F5]/90 to-white/90 border-[#0f1c2c]/10'}
        `}>
          ...
          <div className="relative z-10 flex flex-col items-center justify-center min-h-[300px]">
            <h1 className={`text-sm md:text-lg font-bold tracking-widest uppercase mb-6 ${theme === 'dark' ? 'text-[#d6a54a]' : 'text-[#0f1c2c]/60'}`}>
              آية اليوم
            </h1>

            {dailyAyah ? (
               <div className="max-w-4xl max-auto">
                 <p className={`text-4xl md:text-6xl leading-[2.5] md:leading-[2.8] font-amiri mb-8 drop-shadow-sm ${theme === 'dark' ? 'text-white' : 'text-[#0f1c2c]'}`}>
                   {dailyAyah.text}
                 </p>
                 <div className={`inline-flex items-center gap-2 px-6 py-2 rounded-full border text-sm md:text-lg font-bold font-tajawal
                    ${theme === 'dark' ? 'border-[#d6a54a]/30 text-[#d6a54a] bg-[#d6a54a]/10' : 'border-[#0f1c2c]/20 text-[#0f1c2c] bg-[#0f1c2c]/5'}
                 `}>
            ...
            <div className="mt-12 pt-12 w-full border-t border-dashed border-white/20 flex flex-col items-center">
               ...
               <p className={`text-lg md:text-2xl font-tajawal opacity-80 ${theme === 'dark' ? 'text-[#FAF8F5]' : 'text-[#0f1c2c]'}`}>معرفة تُثمر يقينًا</p>
            </div>
          </div>
        </div>
      </motion.div>
          ...
          {[
            ...
            <Link to={feature.path} key={i}>
              ...
              >
                ...
                <div className={`p-4 rounded-xl mb-4 z-10 transition-colors ${theme === 'dark' ? 'bg-[#d6a54a]/10 text-[#d6a54a]' : 'bg-[#0f1c2c]/10 text-[#0f1c2c]'}`}>
                  {feature.icon}
                </div>
                <h3 className={`text-xl font-bold mb-1 z-10 ${theme === 'dark' ? 'text-white' : 'text-[#0f1c2c]'}`}>{feature.title}</h3>
                <p className={`text-sm z-10 opacity-70 ${theme === 'dark' ? 'text-[#FAF8F5]' : 'text-gray-600'}`}>{feature.desc}</p>
              </motion.div>
            </Link>
          ))}
        </div>
        ...
        <motion.button
          ...
          className={`px-8 py-3 rounded-full font-bold text-sm border flex items-center gap-2 transition-all
            ${theme === 'dark' 
              ...
              : 'border-[#0f1c2c]/40 text-[#0f1c2c] hover:bg-[#0f1c2c]/5'
            }
          `}
        >
          <Sparkles size={16} />
