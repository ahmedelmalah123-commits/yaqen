...
const Yaqeen = () => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center overflow-hidden bg-[#0A1A14]">

      {/* Deep Animated Cosmic Backgrounds */}
      <motion.div
        animate={{
          background: [
            "radial-gradient(circle at 10% 20%, rgba(200,169,106,0.2) 0%, rgba(10,26,20,1) 70%)",
            "radial-gradient(circle at 90% 80%, rgba(200,169,106,0.1) 0%, rgba(10,26,20,1) 80%)",
            "radial-gradient(circle at 50% 10%, rgba(200,169,106,0.25) 0%, rgba(5,15,10,1) 60%)",
            "radial-gradient(circle at 10% 20%, rgba(200,169,106,0.2) 0%, rgba(10,26,20,1) 70%)"
          ]
        }}
      ...
      <ParticlesBackground count={60} color="#F5F5DC" />
      ...
      <div className="absolute top-8 left-8 right-8 flex justify-between z-50">
        ...
        <div className="p-4 rounded-full bg-[#d6a54a]/10 text-[#d6a54a] backdrop-blur-md border border-[#d6a54a]/20 font-bold tracking-widest text-lg flex items-center gap-3 font-reem">
          <div className="w-2.5 h-2.5 rounded-full bg-[#d6a54a] animate-pulse"></div>
          وضع اليقين
      ...
      <AnimatePresence mode="wait">
        {!loading && ayah && (
          ...
          >
            <h3 className="text-[#d6a54a] font-kufi text-2xl md:text-3xl mb-8 md:mb-12 tracking-widest opacity-90 border-b border-[#d6a54a]/30 pb-4">
              سورة {ayah.surah.name.replace(/س(?:ُورَةُ|ورة)\s*/g, '')}
            </h3>

            <h2 className="text-[#F5F5DC] font-amiri text-3xl md:text-[6rem] leading-[2] md:leading-[2.2] tracking-wide drop-shadow-[0_0_30px_rgba(200,169,106,0.4)]" style={{ wordSpacing: '8px' }}>
              {ayah.text}
              <span className="inline-flex items-center justify-center mx-4 md:mx-6 text-2xl md:text-5xl font-black text-[#d6a54a] opacity-70">
            ...
            <motion.button
              ...
              className="mt-12 md:mt-20 w-20 h-20 md:w-24 md:h-24 rounded-full border-2 border-[#d6a54a]/40 flex items-center justify-center text-[#d6a54a] hover:bg-[#d6a54a] hover:text-[#0A1A14] transition-all shadow-[0_0_30px_rgba(200,169,106,0.3)] backdrop-blur-md"
            >
      ...
      {transitioning && (
        <motion.div
          ...
          className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#d6a54a] to-transparent origin-right"
