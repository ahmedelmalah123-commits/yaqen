  ...
  return (
    <div className="py-16 max-w-6xl mx-auto px-4 font-ibm pb-40 relative">
      <div className="mb-20 text-center relative z-10">
        <h1 className={`text-4xl md:text-7xl font-black mb-6 drop-shadow-lg ${theme === 'dark' ? 'text-primary' : 'text-secondary'}`}>
          السيرة النبوية
        </h1>
        <p className={`text-xl md:text-2xl opacity-80 max-w-3xl mx-auto leading-relaxed ${theme === 'dark' ? 'text-white' : 'text-secondary'}`}>
      ...
      <div className="relative z-10">
        ...
        <div className={`absolute left-4 md:left-1/2 top-0 bottom-0 w-1 md:-ml-0.5 rounded-full ${theme === 'dark' ? 'bg-primary/20' : 'bg-secondary/20'}`}></div>
                  ...
                  >
                     ...
                     <div className="relative z-10">
                        {item.media && item.media.type === 'image' && (
                          <div className="w-full h-48 md:h-64 rounded-2xl overflow-hidden mb-6 border border-primary/20 shadow-lg">
                            <img loading="lazy" 
                              src={item.media.src} 
                        ...
                        <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-bold mb-4 ${theme === 'dark' ? 'bg-primary/20 text-primary' : 'bg-secondary/10 text-secondary'}`}>
                          {item.year}
                        </span>
                        
                        <h4 className={`text-xl font-medium mb-2 opacity-80 border-b pb-2 inline-block ${theme === 'dark' ? 'text-primary border-primary/30' : 'text-secondary border-secondary/20'}`}>
                          {item.phase}
                        </h4>
